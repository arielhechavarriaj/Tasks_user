...
import {StatusRegister} from "../../interfaces";
import Swal from "sweetalert2";
import {EmailValidator} from "../../helpers/email.validators";
...
@Component({
  ...
  imports: [
    NgIf, CommonModule, FormsModule, ReactiveFormsModule, FlexModule, RouterLink
  ],
    ...
    this.authService.register(email, password, name).subscribe({
      next: data => {
        if (data.message === StatusRegister.created) {
          ...
          Swal.fire('Error', 'El usuario ya existe en el sistema', 'error').then(() => this.registerForm.reset());
        }
      },
      error: err => {
